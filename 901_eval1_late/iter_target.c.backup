...
launch_result_t solve_launch(const launch_input_t * this_launch,
  ...
  //@@@printf("dest planet speed: %.6f radian / hr\n", 2 * M_PI / dest_planet->year_len);
  ...
  for (size_t i = 0; i < this_launch->max_iterations; ++i) {
    ...
    //double dest_point_rad = atan2(dest.y, dest.x);
    //if (dest_point_rad < 0) dest_point_rad += 2 * M_PI;
    //dest_point_rad = fmod(dest_point_rad, 2*M_PI);
    //if (dest_point_rad == 0) dest_point_rad += 2 * M_PI;
    //@@@printf("(dest point) dest radian: %.6f, angle: %.6f\n", dest_point_rad, dest_point_rad * 180 / M_PI);
    ...
    //@@@printf("dest planet traveled %.6f radian during this period (%.6f)\n", duration * 2*M_PI/dest_planet->year_len, fmod(duration * 2*M_PI / dest_planet->year_len, 2*M_PI));
    ...
    //@@@printf("20hr later (for next input file) rad: %.6f, angle: %.6f\n", next_input, next_input * 180 / M_PI);
